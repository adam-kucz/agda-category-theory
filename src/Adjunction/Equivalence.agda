...
open import Function
  using (Bijection; forw; back; left-inverse; right-inverse)
open import Proof
...
-|→⊣ {ℂ = ℂ}{𝔻}{F}{G} A = record
  ...
  where open _-|_ ⦃ … ⦄
        instance
          ...
          _ = F; _ = G
          _ = A
          _ = λ {X : obj ⦃ ℂ ⦄} {Y : obj ⦃ 𝔻 ⦄} → bijection ⦃ A ⦄ X Y
        F0 = F₀ ⦃ F ⦄
        F1 = F₁ ⦃ F ⦄
        G0 = F₀ ⦃ G ⦄
        G1 = F₁ ⦃ G ⦄
            ...
            〉 _==_ 〉 back (forw (G1 (F1 f) ∘ η₀ X))
              :by: ap back (
                proof forw (η₀ Y ∘ f)
                  ...
                  〉 _==_ 〉 id (F0 Y) ∘ F1 f
                    :by: ap (_∘ F1 f) $ right-inverse _
                  〉 _==_ 〉 F1 f
                    :by: left-unit _
                  〉 _==_ 〉 F1 f ∘ id (F0 X)
                    :by: sym $ right-unit _
                  〉 _==_ 〉 F1 f ∘ forw (η₀ X)
                    :by: ap (F1 f ∘_) $ sym $ right-inverse _
