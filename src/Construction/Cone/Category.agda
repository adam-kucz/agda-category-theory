...
module Construction.Cone.Category ⦃ ℂ : Category 𝒰 𝒱 ⦄ (𝕀 : Category 𝒲 𝒯) where
...
_~>_ ⦃ ConeCat D ⦄ (U , θ) (V , ϕ) = Σₚ λ (f : U ~> V) → ∀ i → ϕ at i ∘ f == θ at i
id ⦃ ConeCat D ⦄ (U , θ) = id U ,, λ i → right-unit (θ at i)
_∘_ ⦃ ConeCat D ⦄ {(U , θ)}{(V , ϕ)}{(W , ψ)}(g ,, p₁)(f ,, p₂) =
  g ∘ f ,, λ i →
    proof ψ at i ∘ (g ∘ f)
      〉 _==_ 〉 ψ at i ∘ g ∘ f :by: assoc _ _ _
      〉 _==_ 〉 ϕ at i ∘ f    :by: ap (_∘ f) $ p₁ i
      〉 _==_ 〉 θ at i       :by: p₂ i
    qed
